...
run_collection () {
    ...
    do
        ...
        python phc/run.py --task HumanoidImMCPGetup --cfg_env phc/data/cfg/phc_shape_mcp_iccv.yaml --cfg_train phc/data/cfg/train/rlg/im_mcp.yaml --motion_file ${amass_train_file} --network_path output/phc_shape_mcp_iccv --test --epoch -1 --im_eval --headless --mode collect --act_noise ${act_noise} --seed ${seed} --num_envs ${num_envs}  --collect_start_idx ${start_idx} --collect_step_idx ${collect_step_idx} --obs_type ${obs_type}
        status=$?
